...
party_cor<- function(df){
  ...
  wordAlk<-c("wodk","vodk","wódk","gin","whiskey","łych","piw","alko","wino")
...
corrplot::corrplot(cor(df_coor[, c(2:6)]),order = 'AOE',method='square', cl.pos = 'n',tl.pos = 'd',
...
#wazna ramka
 df %>%
    select(content) %>% 
    tidytext::unnest_tokens(input=content,output='word',format='text',drop=TRUE, to_lower=TRUE) %>% 
    mutate(first= substr(word, 1, 1),n= nchar(word))  %>% select(first,n) %>% filter(first %in% letters & n<25) %>% 
...
unique_words<- function(df){
  ...
  group_by(year,word) %>% 
  count() %>% ungroup() %>% arrange(year) %>% distinct(word, .keep_all = TRUE) %>% 
  group_by(year) %>% summarise(ni=n())  %>% 
...
get_dfcall<- function(df,who,option){
  ...
  if(option=="suma"){
    df_call %>% group_by(sender_name) %>% summarise(n= (ceiling(sum(call_duration)/36))/100) %>% arrange(-n) %>% mutate(sender_name = sub("(\\w+).*", "\\1",sender_name))}
  if(option=="max"){
    df_call %>% group_by(sender_name) %>% summarise(n= (ceiling(max(call_duration)/36))/100)%>% arrange(-n) %>% mutate(sender_name = sub("(\\w+).*", "\\1",sender_name))}
  if(option=="nieodebrane"){
    df_call[grepl("You missed a call from", df_call$content),] %>%  group_by(sender_name) %>% summarise(n= n())%>% arrange(-n) %>% mutate(sender_name = sub("(\\w+).*", "\\1",sender_name))}
